...
# 依存関係のチェックとインストール
# lxqt を lxde に変更
REQUIRED_PACKAGES="xvfb lxde x11vnc websockify novnc dbus-x11"
PACKAGES_TO_INSTALL=""

for pkg in $REQUIRED_PACKAGES; do
  # パッケージ名とコマンド名が違う場合に対応
  if [ "$pkg" = "lxde" ]; then
    command_to_check="startlxde"
...
echo "🚀 Starting virtual desktop environment (LXDE)..."
...
cleanup() {
  ...
  # lxqt-session を lxsession に変更
  pkill lxsession || true
